...
export function VenuePickerSheet({
  ...
  return (
    <div
      ...
      role="presentation">
      <div
        className="flex max-h-[85vh] w-full max-w-lg flex-col rounded-t-2xl border border-line bg-[#0d0d0d] sm:rounded-2xl"
        ...
        <div className="flex items-center justify-between border-b border-elevated px-5 py-4">
          ...
          <button type="button" onClick={handleClose} className="text-muted hover:text-white">
            <X className="h-5 w-5" />
          </button>
        </div>

        <div className="flex items-center gap-2 border-b border-elevated px-5 py-3">
        ...
        <ul className="flex-1 overflow-y-auto px-3 py-2">
          {filtered.length === 0 ? (
            <li className="px-2 py-8 text-center text-sm text-muted">Nenhuma quadra encontrada</li>
          ) : (
            filtered.map((v) => (
              <li key={v.id}>
                <button
                  ...
                  className="flex w-full items-start gap-3 rounded-xl px-3 py-3 text-left hover:bg-card">
                  <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-lime" />
                  <span>
                    <span className="block font-semibold text-white">{v.name}</span>
                    <span className="block text-xs text-muted">{v.address}</span>
                  </span>
                </button>
              </li>
            ))
        ...
        <div className="border-t border-elevated p-4">
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-lime/50 py-3 text-sm font-bold text-lime">
